...
const VALID_TABS = ["overview", "reviews", "trends", "words"];

/**
 * HomeDashboard — used on the Home page where there is no persistent product ID
 * in the URL. Uses ?tab= query param to track the active tab so each tab is
 * effectively a separate "page" state with a shareable URL.
 */
export default function HomeDashboard({ data }) {
  ...
  const tabs = [
    { id: "home-tab-overview", key: "overview", label: "Overview",                    icon: ChartColumn },
    { id: "home-tab-reviews",  key: "reviews",  label: `Reviews (${reviews.length})`, icon: MessageSquareText },
    { id: "home-tab-trends",   key: "trends",   label: "Trends",                      icon: TrendingUp },
    { id: "home-tab-words",    key: "words",    label: "Word Analysis",                icon: Star },
  ];
  ...
  const insights = [
    { icon: Sparkles,     label: "Dominant sentiment", value: strongestSentiment },
    { icon: ClipboardList, label: "Review coverage",   value: `${coveragePct}%` },
    { icon: ShieldCheck,  label: "Data source",        value: product.source ? String(product.source) : "Combined" },
  ];
        ...
        <div className="dashboard-product">
          ...
          <div className="dashboard-product-copy">
            ...
            <div className="dashboard-product-meta">
              ...
              {product.url && (
                <a href={product.url} target="_blank" rel="noreferrer" className="dashboard-product-link">
          ...
          <div className="dashboard-insights">
            {insights.map(({ icon: Icon, label, value }) => (
              <div key={label} className="dashboard-insight-card">
                <div className="dashboard-insight-icon"><Icon size={16} /></div>
